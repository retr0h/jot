...
#
# jot installer
# Usage: curl -fsSL https://github.com/retr0h/jot/raw/main/install.sh | bash
#
# Env overrides:
#   JOT_VERSION       install a specific version (e.g. 1.1.1) instead of latest
#   JOT_INSTALL_DIR   force install destination, skipping the default rules
...
download_with_progress() {
    ...
    local basename="${tmp_dir}/jot_install_$$"
...
detect_os() {
    ...
    case "$raw" in
        Darwin)  os=darwin ;;
        Linux)   os=linux ;;
        *)       err "unsupported OS: $raw — build from source: https://github.com/retr0h/jot#-build-from-source" ;;
    esac
}
...
resolve_version() {
    if [ -n "${JOT_VERSION:-}" ]; then
        version=${JOT_VERSION#v}
        return
    fi
    tag=$(http_get https://api.github.com/repos/retr0h/jot/releases/latest \
...
resolve_install_dir() {
    ...
    install_dir=$HOME/.jot/bin
    needs_symlink=1
}

setup_tmp() {
    tmp=$(mktemp -d 2>/dev/null || mktemp -d -t jot-install)
    trap 'rm -rf "$tmp"' EXIT
}

download() {
    base=https://github.com/retr0h/jot/releases/download/v${version}
    asset=jot_${version}_${os}_${arch}

    print_message info "\n${MUTED}Installing ${NC}jot ${MUTED}version: ${NC}$version"
    fetch "$base/$asset" "$tmp/jot" progress \
...
verify_checksum() {
    ...
    if have shasum; then
        actual=$(shasum -a 256 "$tmp/jot" | awk '{print $1}')
    elif have sha256sum; then
        actual=$(sha256sum "$tmp/jot" | awk '{print $1}')
    else
        err "neither shasum nor sha256sum found on PATH"
    fi
    if [ "$expected" != "$actual" ]; then
        printf "${RED}jot: checksum mismatch for %s${NC}\n  expected: %s\n  actual:   %s\n" \
...
strip_quarantine() {
    [ "$os" = "darwin" ] || return 0
    xattr -d com.apple.quarantine "$tmp/jot" 2>/dev/null || true
}

install_binary() {
    mkdir -p "$install_dir" || err "cannot create $install_dir"
    install -m 755 "$tmp/jot" "$install_dir/jot" \
        || err "cannot write to $install_dir/jot"
}

maybe_symlink() {
    [ "$needs_symlink" = "1" ] || return 0
    if [ -w /usr/local/bin ]; then
        ln -sf "$install_dir/jot" /usr/local/bin/jot 2>/dev/null || true
    fi
}

print_summary() {
    printf "\n"
    printf "${MUTED}░█ █▀█ ▀█▀${NC}   ${MUTED}installed to${NC} ${ACCENT}%s/jot${NC}\n" "$install_dir"
